...
def draw_HumBurden_Reservoirs(OCN, X, Draw_Borders=True, Draw_River=True, Draw_Frame=True,
                              Borders_Color='black', River_Color='#2776ea',
                              cmap=None, binary=False, seeding = None):
    ...
    # Check whether X is already in the MSC grid; if not, convert it (simplified)
    ...
    if not binary:
        if not np.all(np.isnan(X)):
            ...
            plt.imshow(X.T, cmap=cmap, alpha=0.78)
    ...
    if seeding is not None:
        ...
        if X0.shape == OCN['FD']['A'].shape:
            ...
            for i in range(len(Xtemp)):
                xi = OCN['geometry']['XI'][i] 
                yi = OCN['geometry']['YI'][i]
        ...
        if not binary:
            ...
            plt.imshow(X0.T, origin='lower', alpha=0.4,
                    cmap=(plt.cm.Blues if np.all(X == 0)
                            else cmap))
        ...
        for i in range(len(OCN['FD']['A'])):
            if i != OCN['outlet'] - 1:
                if OCN['FD']['A'].flat[i] >= OCN['thrA']:
                    ...
                    lw = 0.6 * (OCN['FD']['A'].flat[i] * 1e-1 * 1e-8) ** 0.3
    ...
    for i in range(nx-1):
        for j in range(ny-1):
            if MSC[i, j] != MSC[i+1, j]:
                ...
                plt.plot([i + 1-0.5, i + 1-0.5], [j-0.5, j + 1-0.5], color=color, linewidth=0.1)
            if MSC[i, j] != MSC[i, j+1]:
                #plt.plot([i, i + 1], [j + 1, j + 1], color=color, linewidth=0.5)
                plt.plot([i-0.5, i + 1-0.5], [j + 1-0.5, j + 1-0.5], color=color, linewidth=0.1)
